...
class Decoder(nn.Module):
    ...
    def forward(self, z, teacher_joints=None):
        ...
        # For free-roll carry: start at init pose
        prev_free = self.init_angles.unsqueeze(0).expand(B, -1)  # [B,DoF]

        all_joint_outputs, all_object_outputs, all_hidden = [], [], []

        for t in range(self.seq_len):
            # ---- Teacher forcing uses t-1 as input; eval/free-roll uses the carried state ----
            if self.training and (teacher_joints is not None):
                prev_for_input = (self.init_angles.unsqueeze(0).expand(B, -1) if t == 0
                                  else teacher_joints[:, t - 1, :])  # [B,DoF]
            else:
                prev_for_input = prev_free  # [B,DoF]

            # Normalize previous joints for the LSTM input
            prev_joints_norm = (prev_for_input - self.joint_mean) / self.joint_range  # [B,DoF]
            ...
            # Δq integration (soft constraints)
            raw_delta = self.mlp_delta(h_t)  # [B,DoF], unconstrained
            delta_q = raw_delta * self.delta_scale  # [B,DoF]
            current_joints = prev_for_input + delta_q  # integrate vs input state

            # Straight-through clamp for recurrence stability
            ...
            # Carry for next step ONLY in free-roll; with teacher forcing we’ll inject teacher[t] next loop
        ...
        joint_traj_flat = joint_traj.reshape(B * self.seq_len, self.joint_dim).float()
        position_out_flat = self.fk_model(joint_traj_flat)  # should run in fp32
        agent_traj = position_out_flat.view(B, self.seq_len, -1).float()

        # Combine agent (FK) + object and normalize
